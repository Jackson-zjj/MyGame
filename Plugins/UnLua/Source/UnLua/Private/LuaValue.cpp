...
namespace UnLua
{
    ...
    FLuaFunction::~FLuaFunction()
    {
        ...
        {
            // 安全检查：确保 lua_State 仍然有效（未关闭）
            // 当 FLuaEnv 被销毁时，lua_close() 会先执行，之后 AllEnvs 中不再有该 L 的映射
            // 通过 FLuaEnv::FindEnv 检查可以判断 lua_State 是否仍然有效
            if (FLuaEnv::FindEnv(L) != nullptr)
            {
                luaL_unref(L, LUA_REGISTRYINDEX, FunctionRef);
            }
            // 如果 lua_State 已关闭，FunctionRef 会在 lua_close 时自动清理，无需手动释放
        }
    }

    lua_State* FLuaFunction::GetActiveState() const
    {
        return L;
    }
