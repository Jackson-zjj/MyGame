...
// #endif
            ...
            UE_LOG(LogTemp, Log, TEXT("Lua Auto GC Stopped. Switched to Manual Mode."));
        }
    ...
    bool FLuaEnv::IsObjectBound(UObject* Object) const
    {
        return ObjectRegistry ? ObjectRegistry->IsBound(Object) : false;
    }
...
void LuaStack(uint64 LPtr)
{
    lua_State* L = (lua_State*)LPtr;
    if (!L)
    {
        auto& Map = UnLua::FLuaEnv::GetAll();
        for (auto& Pair : Map)
        {
            L = Pair.Key;
            break;
