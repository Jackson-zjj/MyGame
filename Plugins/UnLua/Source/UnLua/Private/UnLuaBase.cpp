...
namespace UnLua
{
    ...
    FString FCheckStack::GetStackDisplayInfo(int depth)
    {
        FString DisplayInfo = "";

        FGenericPlatformStackWalk::InitStackWalking();
        auto CStack = FGenericPlatformStackWalk::GetStack(0, depth);

        for (size_t i = 0; i < CStack.Num(); i++)
        {
            if (FString(CStack[i].ModuleName).Len() > 0)
            {
                FString ModuleName(ANSI_TO_TCHAR(CStack[i].ModuleName));
                int32 Index = 0;
                if (ModuleName.FindLastChar(TEXT('/'), Index) || ModuleName.FindLastChar(TEXT('\\'), Index))
                    ModuleName = ModuleName.Right(ModuleName.Len() - Index - 1);

                DisplayInfo = DisplayInfo + "\n" + FString::Printf(TEXT("%s!%s Line %d"), *ModuleName, ANSI_TO_TCHAR(CStack[i].FunctionName), CStack[i].LineNumber);
            }
        }
    ...
    FCheckStack::FCheckStack(lua_State* InL)
        : L(InL), ExpectedTopIncrease(0)
    {
        OldTop = lua_gettop(L);
    }

    FCheckStack::FCheckStack(lua_State* InL, int32 _ExpectedTopIncrease)
        : L(InL), ExpectedTopIncrease(_ExpectedTopIncrease)
    {
        OldTop = lua_gettop(L);
    }
    ...
    FCheckStack::~FCheckStack()
    {
        ...
        {
            FString StackInfo = GetStackDisplayInfo(5);
            UE_LOG(LogUnLua, Error, TEXT("Lua Top Increase : %d \n, Stack:%s"), Top - OldTop - ExpectedTopIncrease, *StackInfo);
        }
        L = nullptr;
    }
