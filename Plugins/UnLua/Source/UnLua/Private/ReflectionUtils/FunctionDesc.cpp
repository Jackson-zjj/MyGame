...
int32 FFunctionDesc::PostCall(lua_State * L, int32 NumParams, int32 FirstParamIndex, void* Params, const FFlagArray& CleanupFlags)
{
    ...
    // --- 新增优化逻辑开始 ---
    bool bHandledReturnWithCache = false;

    // 只有当存在返回值，且返回值是 Struct 类型时才尝试优化
    if (ReturnPropertyIndex > INDEX_NONE)
    {
        const auto& ReturnPropDesc = Properties[ReturnPropertyIndex];
        FProperty* RawProp = ReturnPropDesc->GetProperty();
        
        if (FStructProperty* StructProp = CastField<FStructProperty>(RawProp))
        {
            // 计算 Lua 传入的参数总数
            int32 LuaArgCount = lua_gettop(L);
            // FirstParamIndex 是 Lua 栈中第一个参数的索引（通常是 2，因为 1 是 self）
            // NumParams 是 C++ 函数需要的参数数量
            // 如果 Lua 传入的数量 = 需要的数量 + 1，说明可能传入了 Cache
            int32 PassedArgs = LuaArgCount - FirstParamIndex + 1;
            int32 RealFuncParamNum = Properties.Num() - (ReturnPropertyIndex > 0 ? 1 : 0);
            if (PassedArgs == RealFuncParamNum + 1)
            {
                int32 CacheIndex = LuaArgCount; // 栈顶就是 Cache

                void* CacheDestBuffer = GetCppInstance(L, CacheIndex);
#if UE_BUILD_DEVELOPMENT
                int32 UserdataRawSize = GetUserdataRawSize(CacheDestBuffer);
                ensureMsgf(UserdataRawSize == StructProp->Struct->PropertiesSize,
                    TEXT("Cache Userdata size(%d) not match(%d), Please check your code!!!"), UserdataRawSize, StructProp->Struct->PropertiesSize);
#endif
                if (CacheDestBuffer)
                {
                    // 执行内存拷贝：将 C++ 返回值 (Params + Offset) 拷贝到 Lua Userdata (CacheDestBuffer)
                    void* SrcValue = StructProp->ContainerPtrToValuePtr<void>(Params);
                    
                    // 使用 CopyScriptStruct 确保深拷贝和正确的析构/构造
                    StructProp->Struct->CopyScriptStruct(CacheDestBuffer, SrcValue);
                    
                    // 将 Cache 对象重新压入栈顶作为返回值
                    lua_pushvalue(L, CacheIndex);
                    
                    bHandledReturnWithCache = true;
                    NumReturnValues++;
                }
            }
        }
    ...
    if (ReturnPropertyIndex > INDEX_NONE)
    {
        // 如果我们已经用 Cache 处理了返回值，就跳过默认逻辑
        if (!bHandledReturnWithCache)
        {
