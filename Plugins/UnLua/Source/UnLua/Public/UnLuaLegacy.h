...
namespace UnLua
{
    ...
    FORCEINLINE int32 Push(lua_State* L, const FVariant& V, bool bCopy = false)
    {
        switch (V.GetType())
        {
        case EVariantTypes::Bool:
            lua_pushboolean(L, V.GetValue<bool>());
            break;
        case EVariantTypes::Int32:
            lua_pushinteger(L, V.GetValue<int32>());
            break;
        case EVariantTypes::Int64:
            lua_pushinteger(L, V.GetValue<int64>());
            break;
        case EVariantTypes::Double:
            lua_pushnumber(L, V.GetValue<double>());
            break;
        case EVariantTypes::Vector:
            Push(L, V.GetValue<FVector>(), bCopy);
            break;
        case EVariantTypes::Rotator:
            Push(L, V.GetValue<FRotator>(), bCopy);
            break;
        case EVariantTypes::Vector2d:
            Push(L, V.GetValue<FVector2D>(), bCopy);
            break;
        case EVariantTypes::String:
            Push(L, V.GetValue<FString>(), bCopy);
            break;
        case EVariantTypes::Name:
            Push(L, V.GetValue<FName>(), bCopy);
            break;
        default:
            ensureAlwaysMsgf(false, TEXT("Unsupported FVariant type: %d"), (int32)V.GetType());
            lua_pushnil(L);
            break;
        }
        return 1;
    ...
    FORCEINLINE_DEBUGGABLE FLuaRetValues CallFunctionInternalWithArrayParam(lua_State* L, const TArray<T1>& Args1, T2&&... Args2)
    {
        // make sure the function is on the top of the stack, and the message handler is below the function
        int32 MessageHandlerIdx = lua_gettop(L) - 1;
        check(MessageHandlerIdx > 0);
        int32 NumArgs = PushArgs<false>(L, Forward<T2>(Args2)...);
        for (const T1& Arg : Args1)
        {
            NumArgs += UnLua::Push(L, Arg);
        }
        int32 Code = lua_pcall(L, NumArgs, LUA_MULTRET, MessageHandlerIdx);
        int32 TopIdx = lua_gettop(L);
        if (Code == LUA_OK)
    ...
    template <typename T1, typename... T2>
    FLuaRetValues FLuaFunction::CallWithArrayParams(const TArray<T1>& Args1, T2&&... Args2) const
    {
        if (!IsValid())
        {
            return FLuaRetValues(L, INDEX_NONE);
        ...
        if (Type != LUA_TFUNCTION)
        {
            UE_LOG(LogUnLua, Warning, TEXT("Function::CallWithArrayParams Is not Function. %s"), ANSI_TO_TCHAR(__FUNCTION__));
            lua_pop(L, 2);
            return FLuaRetValues(L, INDEX_NONE);
        }
        
        return CallFunctionInternalWithArrayParam(L, Args1, Forward<T2>(Args2)...);
    }
